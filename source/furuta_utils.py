...
def read_yaml_parameters(folder_path: str, yaml_file: str = None) -> dict:
    ...
    try:
        with open(yaml_path, 'r') as params_file:
            ...
            parameters['callbacks']['model_checkpoint']['filepath'] = parameters['callbacks']['model_checkpoint']['filepath'].replace('model_name', parameters['training']['model_name'])
            parameters['callbacks']['tensor_board']['log_dir'] = parameters['callbacks']['tensor_board']['log_dir'].replace('model_name', parameters['training']['model_name'])
